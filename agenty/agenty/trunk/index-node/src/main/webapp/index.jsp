...
<body style="text-align: left">
	...
	<%
		URL indexURL = new URL(request.getScheme(),
				request.getServerName(), request.getServerPort(),
				request.getContextPath() + "/Index?wsdl");
	...
	<div class="frame">
		...
		<div class="body">
			<table>
				...
				<%
					for (IndexEntry entry : dataSources)
							if (DataSourceType.SPARQL_ENDPOINT.equals(entry.getType())) {
				%>
				<tr>
					<td><a href="
							<%=ontology%>"><%=ontology%> </a></td>
					<td><a href="
...
<%-- 					<td>
					...
					<td>
						<form action="indexService" method="post"> 
							<input type="hidden" name="index_url" id="index_url" value="<%=indexURL.toString()%>"> 
							<input type="hidden" name="toggle_url" id="toggle_url" value="<%=entry.getUrl()%>"> 
							<input type="hidden" name="returnTo" id="returnTo" value="index.jsp"> 
							<input type="hidden" name="action_type" id="action_type" value="toggle">
					...
					<td>
						<form action="indexService" method="post">
							<input type="hidden" name="remove_ontology_url"
								id="remove_ontology_url" value="<%=ontology%>"> <input
								type="hidden" name="remove_endpoint_url"
								id="remove_endpoint_url" value="<%=entry.getUrl()%>"> <input
								type="hidden" name="no_write" id="no_write" value="true">
							<input type="hidden" name="returnTo" id="returnTo"
								value="index.jsp"> <input type="submit" value="Delete">
						</form>
					</td>
					<td>
						<button
							onclick="openEditForm('<%=ontology%>','<%=entry.getUrl()%>','Endpoint');">
							Edit</button>
					</td>
				</tr>
				<%
				...
				<tr>
					...
					<td>
						<form action="indexService" method="post">
							<input type="hidden" name="remove_type" id="remove_type"
								value="TURTLE"> <input type="hidden" name="remove_url"
								id="remove_url" value="<%=entry.getUrl()%>"> <input
								type="hidden" name="no_write" id="no_write" value="true">
							<input type="hidden" name="returnTo" id="returnTo"
								value="index.jsp"> <input type="submit" value="Delete">
						</form>
					...
					<td>
						<button
							onclick="openEditForm('<%=ontology%>','<%=entry.getUrl()%>','Graph');">
							Edit</button>
					</td>
				</tr>
				<%
