...
class Process:
	...
	ifOpt = re.compile("(?i) (\w+) (\w+) (\w+) :(\w+)")
	elseOpt = re.compile("(?i) :(\w+)")
	...
	def proces(self, plik):
		...
		for m in self.nawiasy.findall(text):
			
			out = ":nawias"+str(n_c)+ "\n"+ m + ":nawias"+str(n_c) + "end\n";

			#print out

			text = text.replace("("+m+")", out)
			n_c = n_c + 1; 
		  ...
		  if len(line) > 1:
			...
			if line.startswith("for"):
				...
				if(m):	
					...
					if var > int(m.group(4)): line = "goto " + m.group(5) + "end"
			...
			if(line.startswith("else")):
				...
				ok = 1
				ala = 0
				m = self.elseOpt.match(line[4:])
				if m:
					if self.in_else == 0:
						line = "goto " + m.group(1)+"end"
			...
			if(not ok):
				print "Bad command or file name", line
				print line
				ala =0
