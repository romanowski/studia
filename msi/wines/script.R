...
net.start <- newff(n.neurons=c(11,6,1),      
...
result <- train(net.start, data, 
	res, error.criterium="LMS", 
	report=TRUE, show.step=500, n.shows=20)
...
error <- abs(netOut - testRes)
jpeg('plots/plot.jpg')
plot(error)
dev.off()

jpeg('plots/plot-bool.jpg')
plot(error > 0.5)
dev.off()
